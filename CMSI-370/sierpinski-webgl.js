...
var startSierpinski = function (canvas) {
	...
	// Dragging the mouse results in translation if the user is holding down the Shift key
	// Set up event handlers: SHIFT + drag-to-translate
	cameraTranslate = function (event) {

		//where to start translate + or - 0.05*(difference between starting coordinate and current mouse coordinate) 
	    viewerLocation.x = xTranslationStart + 0.05*(xDragStart - event.clientX);  
        viewerLocation.y = yTranslationStart - 0.05*(yDragStart - event.clientY);	
        drawScene();
		
    };
	
	
	// Dragging the mouse results in scaling if the user is holding down the Alt/Option key.
	// Set up event handlers: ALT/OPTION + drag-to-scale
	cameraScale = function (event) {
	
	        //current scale factor - 0.01*(differeence between starting y coordinate and current mouse y coordinate)
		    scaleFactor = scaleFactor + 0.01*(yDragStart - event.clientY); //only need y coordinate
			scaleFactor = scaleFactor <= 0 ? 0 : scaleFactor; // prevent losing the triangle
    ...
    canvas.onmousedown = function (event) {
		...
		xTranslationStart = viewerLocation.x;
		yTranslationStart = viewerLocation.y;
		
		// translate if shift key is held
		if (event.shiftKey) {
		    canvas.onmousemove = cameraTranslate;
		}
		
		// scale if alt/option key is held
		else if (event.altKey) {
		    canvas.onmousemove = cameraScale;
		}
